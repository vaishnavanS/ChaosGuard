...
export default function Metrics() {
  ...
  const { data: containersRes } = useQuery({
    ...
    refetchInterval: 2500, // Query resources every 2.5 seconds
  });
  ...
  // Maintain sliding window history of resources
  useEffect(() => {
    const timeLabel = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' });
    setHistory(prev => {
      const next = [...prev, { time: timeLabel, totalCpu, totalMemory: totalMemoryMB }];
      if (next.length > 15) return next.slice(1); // Keep last 15 ticks
      return next;
    });
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="p-6 rounded-xl border border-slate-800 bg-[#0f172a]/20 flex items-center justify-between">
          <div>
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Active Attacks (Gauge)</span>
            <h3 className="text-3xl font-extrabold text-amber-500 mt-1">{promActiveAttacks}</h3>
            <p className="text-[10px] text-slate-400 mt-1 font-mono">chaosguard_active_attacks</p>
          </div>
        ...
        <div className="p-6 rounded-xl border border-slate-800 bg-[#0f172a]/20 flex items-center justify-between">
          <div>
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Containers Monitored</span>
            <h3 className="text-3xl font-extrabold text-violet-400 mt-1">{promMonitored}</h3>
            <p className="text-[10px] text-slate-400 mt-1 font-mono">chaosguard_containers_monitored</p>
          </div>
        ...
        <div className="p-6 rounded-xl border border-slate-800 bg-[#0f172a]/20 flex items-center justify-between">
          <div>
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Total Fault Injections</span>
            <h3 className="text-3xl font-extrabold text-emerald-400 mt-1">{promTotalAttacks}</h3>
            <p className="text-[10px] text-slate-400 mt-1 font-mono">chaosguard_attacks_total</p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="p-6 rounded-xl border border-slate-800 bg-slate-900/10 flex flex-col">
          <div className="flex items-center gap-2 mb-6">
            <Cpu className="h-5 w-5 text-indigo-400" />
            <h2 className="text-base font-semibold tracking-tight">Aggregate CPU Load (%)</h2>
          </div>
            ...
            {history.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={history}>
                  ...
                  />
                  <Area type="monotone" dataKey="totalCpu" stroke="#6366f1" fillOpacity={1} fill="url(#cpuGrad)" />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-full flex items-center justify-center text-xs text-slate-500">
                Initializing timeline metric graph...
              </div>
            )}
          </div>
        </div>
        ...
        <div className="p-6 rounded-xl border border-slate-800 bg-slate-900/10 flex flex-col">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-base font-semibold tracking-tight">Aggregate Memory Memory (MB)</h2>
          </div>
          <div className="h-72 w-full flex-1">
            {history.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={history}>
                  ...
                  <Tooltip 
                    contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '8px', color: '#f8fafc' }}
                  />
                  <Area type="monotone" dataKey="totalMemory" stroke="#0d9488" fillOpacity={1} fill="url(#memGrad)" />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-full flex items-center justify-center text-xs text-slate-500">
      ...
      <div className="p-6 rounded-xl border border-slate-800 bg-slate-900/10">
        <div className="flex items-center gap-2 mb-6">
          <Shuffle className="h-5 w-5 text-violet-400" />
          <h2 className="text-base font-semibold tracking-tight">Individual Container Resource Allocation</h2>
        </div>
